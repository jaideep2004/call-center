...
async function main() {
  const pub = await pool.query('SELECT id, name, user_id, active, deleted_at FROM app.publishers ORDER BY created_at DESC LIMIT 20');
  console.log('Publishers:');
  console.table(pub.rows);
  const users = await pool.query('SELECT id, email, role FROM "user" WHERE role IN (\'publisher\', \'agent\') ORDER BY created_at DESC LIMIT 20');
