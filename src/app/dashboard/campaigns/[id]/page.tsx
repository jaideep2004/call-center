...
function CampaignDetailInner() {
	...
	const activeTab: TabKey = (
		rawTab && TAB_KEYS.has(rawTab as TabKey) ? rawTab : "general"
	) as TabKey;
	...
	function setTab(key: TabKey) {
		...
		qs.set("tab", key);
		router.push(`/dashboard/campaigns/${id}?${qs.toString()}`, {
			scroll: false,
		});
	}
		...
		fetch(`/api/v1/campaigns/${id}/bid`)
			.then(async (res) => {
				if (res.ok) {
					...
					if (o) {
						setBidPrice(
							o.price_cents != null ? String(o.price_cents / 100) : "",
						);
						setBidPayout(
							o.payout_cents != null ? String(o.payout_cents / 100) : "",
						);
						setBidNote(o.note ?? "");
					}
				}
	...
	async function saveBid() {
		...
		try {
			const res = await fetch(`/api/v1/campaigns/${id}/bid`, {
				...
				body: JSON.stringify({
					price_cents:
						bidPrice === "" ? null : Math.round(parseFloat(bidPrice) * 100),
					payout_cents:
						bidPayout === "" ? null : Math.round(parseFloat(bidPayout) * 100),
					note: bidNote || null,
				}),
			});
			const body = await res.json();
	...
	useEffect(() => {
		fetch(`/api/v1/campaigns/${id}/assignments`)
			.then(async (res) => {
				if (res.ok) {
					const body = await res.json();
	...
	useEffect(() => {
		fetch(`/api/v1/campaigns/${id}/publishers`)
			.then(async (res) => {
				if (res.ok) {
					const body = await res.json();
	...
	function toggleAssignment(kind: "agency" | "agent", value: string) {
		...
		const key =
			kind === "agency" ? "assigned_agency_ids" : "assigned_agent_ids";
	...
	function toggleArray(field: "allowed_endpoints", value: string) {
		if (!campaign) return;
	...
	async function deployToRetreaver() {
		setDeploying(true);
		try {
			const res = await fetch(`/api/v1/campaigns/${id}/retreaver`, {
				method: "POST",
			});
			const body = await res.json();
			if (res.ok) {
	...
	return (
		<div className='dashboard-page'>
			...
			<p
				...
				style={{ fontSize: 10, margin: "6px 2px 0", letterSpacing: "0.4px" }}>
				{activeTab === "general" &&
					"Routing, price, status, and recording — the campaign core."}
				{activeTab === "publishers" &&
					"Multi-select which publishers can send traffic — uses campaign_publishers."}
				{activeTab === "bidding" &&
					"Manual bid overrides apply immediately to billing and payout."}
				{activeTab === "rtb" &&
					"Real-time bidding and Retreaver intake numbers."}
				{activeTab === "assignments" &&
					"Agencies, agents, endpoints, and policy."}
			</p>
			...
			<div style={{ marginTop: "var(--space-5)" }}>
								...
								<dl className='data-list'>
									...
									<dd style={{ margin: "10px 0px" }}>
										<select
											...
											onChange={(e) =>
												update("routing_strategy", e.target.value)
											}
											style={{ maxWidth: 160 }}>
									...
									<dd style={{ margin: "10px 0px" }}>
										<label className='toggle'>
											<input
												...
												onChange={(e) =>
													update("record_calls", e.target.checked)
												}
											/>
				...
				{activeTab === "publishers" && (
					<section
						className='card'
						style={{ padding: "var(--space-6)", maxWidth: 560 }}>
						<div
							style={{
								display: "flex",
								alignItems: "center",
							...
							<span className='badge' style={{ fontSize: 10 }}>
								{campaignPublishers.length} selected
							</span>
							<span
								className='text-muted text-mono-sm'
								style={{ marginLeft: "auto", fontSize: 10 }}>
								campaign_publishers join • multi
							</span>
						</div>
						<p
							...
							Select all publishers that can send traffic for this campaign.
							Stored in{" "}
							<span style={{ color: "var(--ink)", fontWeight: 600 }}>
								campaign_publishers
							</span>{" "}
							(legacy <span className='text-mono-sm'>publisher_id</span> stays
							synced to first).
						</p>
						<div
							style={{
								...
								borderRadius: 10,
								padding: 12,
								background: "rgba(22,12,42,.35)",
							}}>
							{publishers.length === 0 ? (
								<span
									className='text-muted text-mono-sm'
									style={{ fontSize: 11 }}>
									No publishers yet — create one in Admin → Publishers.
								</span>
							) : (
								publishers.map((p) => {
									...
									return (
										<label
											...
											style={{
												...
												border:
													"1px solid " +
													(checked ? "rgba(168,85,247,.28)" : "transparent"),
												background: checked
													? "rgba(168,85,247,.13)"
													: "transparent",
											}}>
											<input
												...
												onChange={(e) => {
													const next = e.target.checked
														? [...campaignPublishers, p.id]
														: campaignPublishers.filter((id) => id !== p.id);
						...
						<div
							...
							<button
								...
								disabled={publishersSaving}
								style={{ minWidth: 136, justifyContent: "center" }}>
								{publishersSaving ? (
									<span className='spinner' />
								) : (
									"Save publishers"
							...
							<span
								className='text-muted text-mono-sm'
								style={{ fontSize: 10 }}>
								Join table persists immediately; legacy publisher_id syncs to
								first.
							</span>
						</div>
					</section>
				...
				{activeTab === "bidding" && (
					<section
						className='card'
						style={{ padding: "var(--space-6)", maxWidth: 640 }}>
						<h2
							style={{
								font: "500 18px var(--serif)",
								margin: "0 0 var(--space-4)",
						...
						<p
							className='text-muted'
							style={{ fontSize: 11, margin: "0 0 var(--space-4)" }}>
							Manual bid — applies immediately to billing and publisher payout.
							No schedule. Leave empty to use campaign price. Max payout caps
							what the platform can pay the publisher for this offer.
						</p>
						<p
						...
						<div
							...
							<label
								...
								<input
									...
									value={
										campaign.max_publisher_payout_cents != null
											? String(campaign.max_publisher_payout_cents / 100)
											: ""
									}
									onChange={(e) =>
										update(
						...
						{bidOverride && (
							<div
								...
								<p
									className='text-mono-sm'
									style={{ fontSize: 11, margin: 0, color: "var(--muted)" }}>
									Active override:{" "}
									<span style={{ color: "var(--ink)" }}>
										{bidOverride.price_cents != null
											? `$${(bidOverride.price_cents / 100).toFixed(2)}/call`
											: "—"}
									</span>{" "}
									• payout{" "}
									<span style={{ color: "var(--ink)" }}>
										{bidOverride.payout_cents != null
											? `$${(bidOverride.payout_cents / 100).toFixed(2)}`
											: "—"}
									</span>{" "}
									{bidOverride.note ? `• ${bidOverride.note}` : ""}
								</p>
							</div>
						)}
						{(campaign.visibility === "exclusive" || campaign.is_exclusive) && (
							<div
								...
								<p
									...
									Exclusive access — only selected agencies/agents see and
									receive this campaign
								</p>
								<p
									className='text-muted'
									style={{ fontSize: 11, margin: "0 0 12px" }}>
									Everyone else is hidden from browse, portal offers, ping and
									routing. Save below to apply.
								</p>
									...
									<>
										<div
											...
											<label
												...
												<select
													...
													size={Math.min(
														5,
														Math.max(3, assignments.agencies.length),
													)}
													...
													onChange={(e) => {
														...
														selected.forEach((v) => {
															if (!assignments.assigned_agency_ids.includes(v))
																toggleAssignment("agency", v);
														});
														assignments.assigned_agency_ids.forEach((v) => {
											...
											<label
												...
												<select
													...
													size={Math.min(
														5,
														Math.max(3, assignments.agents.length),
													)}
													...
													onChange={(e) => {
														...
														selected.forEach((v) => {
															if (!assignments.assigned_agent_ids.includes(v))
																toggleAssignment("agent", v);
														});
														assignments.assigned_agent_ids.forEach((v) => {
															if (!selected.includes(v))
																toggleAssignment("agent", v);
														});
													}}
										...
										<div
											...
											<span className='text-muted' style={{ fontSize: 11 }}>
												{assignments.assigned_agency_ids.length} agencies ·{" "}
												{assignments.assigned_agent_ids.length} agents selected
											</span>
										</div>
									</>
								)}
					...
					<div className='camp-grid'>
								...
								<dl className='data-list'>
									...
									<dd style={{ margin: "10px 0px" }}>
										<label className='toggle'>
											<input
												...
												onChange={(e) =>
													update("rtb_enabled", e.target.checked)
												}
											/>
										...
										<p
											...
											Routes Retreaver bid requests to this campaign&apos;s
											Telnyx number.
										</p>
									</dd>
									<dt>Postback key</dt>
									<dd style={{ margin: "10px 0px" }}>
										...
										<p
											className='text-muted'
											style={{ fontSize: 11, margin: "8px 0 0" }}>
											Paste the key from Retreaver (campaign → Postback Keys →
											Real Time Bidding). Stored encrypted with ENCRYPTION_KEY.
						...
						<div className='stack' style={{ gap: "var(--space-5)" }}>
							<section className='card' style={{ padding: "var(--space-6)" }}>
								...
								{campaign.retreaver_cid ? (
									...
									<p className='text-muted' style={{ fontSize: 12, margin: 0 }}>
										Deploy to Retreaver or run the campaign sync to link this
										campaign first.
									</p>
								)}
							</section>
							<section className='card' style={{ padding: "var(--space-6)" }}>
								...
								<p
									className='text-muted'
									style={{ fontSize: 12, margin: "0 0 var(--space-4)" }}>
									Telnyx DIDs that route inbound calls to this campaign. Inbound
									only matches active numbers.
								</p>
								{trackingLoading ? (
									<div className='stack' style={{ gap: 8 }}>
										<div className='skeleton skeleton-text' />
									...
									<p className='text-muted' style={{ fontSize: 12, margin: 0 }}>
										No tracking numbers on this campaign yet
										{isAdmin ? " — add the first one below." : "."}
									</p>
								) : (
									<div className='data-table-wrap'>
										<table className='data-table'>
								...
								{assignmentLoading ? (
									...
									<div className='stack' style={{ gap: "var(--space-4)" }}>
										...
										<div>
											<p
												...
												<span
													...
													— agents missing any ticked skill are skipped
													(skill_missing); empty = no skill filter
												</span>
											</p>
											<div
												className='stack-h'
												...
												{skillOptions.length === 0 && (
													<span className='text-muted' style={{ fontSize: 12 }}>
														No verticals yet — create one from Agents → Invite →
														+ New vertical.
													</span>
												)}
												{skillOptions.map((s) => {
													const on = (campaign?.required_skills ?? []).includes(
														s,
													);
													return (
														<button
										...
										<div
											...
											<span className='text-muted' style={{ fontSize: 10 }}>
												No selections = all agencies/agents eligible (open
												campaign).
											</span>
										</div>
									</div>
