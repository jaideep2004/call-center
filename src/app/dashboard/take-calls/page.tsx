...
function TakeCallsInner() {
	...
	const [callDebounced, setCallDebounced] = useState(
		searchParams.get("q") ?? "",
	);
	...
	useEffect(() => {
		if (!agentId) return;
		fetch(
			`/api/v1/calls?agent_id=${encodeURIComponent(agentId)}&limit=50`,
		).then(async (res) => {
			if (res.ok) {
				const body = await res.json();
				setCalls(body.data ?? []);
	...
	useEffect(() => {
		const t = setTimeout(() => {
			...
			if (trimmed !== callDebounced) {
				setCallDebounced(trimmed);
				setCallPage(1);
			}
		}, 300);
	...
	const fetchLiveCampaigns = useCallback(async () => {
		const res = await fetch("/api/v1/agent/campaigns?status=active&limit=100");
		if (!res.ok) return;
		const body = await res.json();
		setLiveCampaigns(
			(body.data ?? []).map(
				(c: { id: string; name: string; is_live_for_me?: boolean }) => ({
					id: c.id,
					name: c.name,
					is_live_for_me: c.is_live_for_me ?? false,
				}),
			),
		);
	}, []);
	...
	const toggleLiveCampaign = useCallback(
		async (campaignId: string, next: boolean) => {
			setTogglingCampaign(campaignId);
			try {
				const res = await fetch(`/api/v1/agent/campaigns/${campaignId}/live`, {
					method: "POST",
				...
				if (res.ok) {
					setLiveCampaigns((prev) =>
						prev.map((c) =>
							c.id === campaignId ? { ...c, is_live_for_me: next } : c,
						),
					);
					showToast(
						next
							? "You are live for this campaign"
							: "Paused for this campaign",
						"success",
					);
				} else {
					...
					showToast(body.message ?? "Failed to update campaign", "error");
				}
			} catch {
	...
	const toggleAvailability = useCallback(async () => {
		...
		try {
			...
			if (res.ok) {
				...
				showToast(
					`You are now ${next === "available" ? "online" : "offline"}`,
					"success",
				);
			} else {
				const body = await res.json();
	...
	const filteredCalls = useMemo(() => {
		...
		return calls.filter(
			(c) =>
				...
				(campaignMap[c.campaign_id] ?? "").toLowerCase().includes(q),
		);
	...
	const paginatedCalls = useMemo(
		() => filteredCalls.slice((callPage - 1) * PAGE_SIZE, callPage * PAGE_SIZE),
	...
	const callColumns: Column<CallRow>[] = [
		{
			...
			header: "Call",
			render: (c) => (
				<Link href={`/dashboard/calls/${c.id}`} className='clickable'>
					{c.id.slice(0, 8)}
				</Link>
			),
		},
		{
			...
			header: "Status",
			render: (c) => (
				<span className={`badge ${CALL_STATE_COLORS[c.state] ?? ""}`}>
					{c.state}
				</span>
			),
		},
		{
			...
			header: "From",
			render: (c) => (
				<span className='text-mono-sm' title={c.from_hash ?? ""}>
					{c.from_hash?.slice(0, 12) ?? "—"}
				</span>
			),
		},
		{
			...
			render: (c) =>
				c.caller_state ? (
					<span className='badge badge-info'>{c.caller_state}</span>
				) : (
		...
		},
		{
			...
			render: (c) => (
				<span className='text-mono-sm'>
					{campaignMap[c.campaign_id] ?? c.campaign_id.slice(0, 8)}
				</span>
			),
		},
		{
	...
	const readyChecks = [
		...
		},
		{
			...
			label: "Mic & Speaker",
			desc: deviceReady
				? "Both tested — verified on this device"
				: "Test mic & speaker to unlock Go Online",
		...
		},
		{
			...
			desc: campaignsReady
				? `Live for ${liveCount} campaign${liveCount === 1 ? "" : "s"} — routing enabled`
				: "Select at least one campaign below to go live",
			meta: campaignsReady ? `${liveCount} live` : "Required",
		},
		{
			ok: statesReady,
			label: "Licensed States",
			desc: agentInfo?.states?.length
				? `${agentInfo.states.length} states selected — routing filtered`
				: "Any state — you will receive calls from all states",
			meta: agentInfo?.states?.length
		...
		},
		{
			...
			label: "Call Endpoint",
			desc: !hasEndpoint
				? "No endpoint configured — contact admin"
				: !pstnForwardingOk
					? "PSTN selected but forwarding number missing"
					: hasWebRTC && hasPSTN
						? `Hybrid: webrtc + pstn → ${agentInfo?.forwarding_number ?? ""}`
						: hasWebRTC
							? `Browser softphone · ${webrtcLabel}`
							: `Phone forward → ${agentInfo?.forwarding_number ?? ""}`,
			meta: endpointReady
				? hasWebRTC && hasPSTN
					? "Hybrid"
					: hasWebRTC
		...
		<div className='dashboard-page tc-page'>
			...
			<section className='tc-hero' aria-labelledby='tc-title'>
				<div className='tc-hero__main'>
					<p className='eyebrow'>
						<i aria-hidden /> AGENT / TAKE CALLS
					</p>
					<h1 id='tc-title'>Take Calls</h1>
					<p className='tc-subtitle'>
						Go online only when every check is green. Devices are verified on
						this browser — refresh keeps your test.
					</p>
					<div className='tc-hero__meta' role='list'>
						...
						<span
							className='badge'
							role='listitem'
							title={agentInfo?.endpoint_types?.join(", ") ?? "No endpoint"}
							style={{ textTransform: "uppercase", letterSpacing: 0.6 }}>
							{agentInfo?.endpoint_types?.length
								? agentInfo.endpoint_types.join(" · ")
								: "No endpoint"}
						</span>
						{hasPSTN && agentInfo?.forwarding_number && (
							<span
								className='text-mono-sm tc-hero__forwarding'
								role='listitem'
								title={agentInfo.forwarding_number}>
								→ {agentInfo.forwarding_number}
							</span>
						)}
						<span className='text-mono-sm tc-hero__count' role='listitem'>
							{calls.length} total calls
						</span>
						{hasWebRTC && (
							<span
								className={`badge ${webrtc.isReady ? "badge-success" : webrtc.error ? "badge-danger" : "badge-warning"}`}
								style={{ fontSize: 9 }}
								title={webrtc.error ?? webrtcLabel}>
								{webrtcLabel}
							</span>
						)}
					</div>
				...
				<div className='tc-hero__cta'>
					<div className='tc-availability' aria-live='polite'>
						<span
							className='take-calls-dot'
							style={{
								background: STATUS_COLORS[availability],
								boxShadow: isOnline ? "0 0 12px rgba(34,197,94,0.45)" : "none",
							}}
							aria-hidden
						/>
						<div style={{ flex: 1, minWidth: 0 }}>
							<p className='take-calls-label' style={{ margin: 0 }}>
								Availability
							</p>
							<p
								className='take-calls-value'
								style={{
									margin: "2px 0 0",
									color: isOnline ? "var(--success, #22c55e)" : "var(--ink)",
								}}>
					...
					{agentId ? (
						<>
							<div
								...
								title={goOnlineBlockedReason ?? undefined}>
								<button
							...
							<p id='tc-cta-help' className='tc-cta-help'>
								{isOnline
									...
									: goOnlineBlockedReason === "Test mic & speaker first"
										...
										: (goOnlineBlockedReason ??
											"Complete all checks to go online.")}
							</p>
							{!isOnline && !allReady && (
					...
					{!isApproved && agentId && (
						<div className='tc-callout tc-callout--warning' role='status'>
							...
							<span>
								Your profile is {agentInfo?.approval_status ?? "pending"}.
								Contact admin to approve before you can receive calls.
							</span>
						</div>
					)}
					{isApproved && !endpointReady && agentId && (
						<div className='tc-callout tc-callout--warning' role='status'>
							...
							<span>
								{hasPSTN && !pstnForwardingOk
									? "PSTN endpoint needs a forwarding number — contact admin."
									: "No call endpoint configured — contact admin to set WebRTC or PSTN."}
							</span>
						</div>
					)}
					{isApproved && endpointReady && !isOnline && (
						<div
							className={`tc-callout ${allReady ? "tc-callout--success" : "tc-callout--muted"}`}
							role='status'>
							<strong>
								{allReady ? "Ready to receive calls" : "Almost ready"}
							</strong>
							<span>
								{allReady
					...
					{isApproved && isOnline && (
						<div className='tc-callout tc-callout--success' role='status'>
							...
							<span>
								Incoming calls will be routed to{" "}
								{hasWebRTC && hasPSTN
									? "browser or phone"
									: hasWebRTC
										? "your browser softphone"
										: hasPSTN
											? (agentInfo?.forwarding_number ??
			...
			<div className='tc-grid'>
				...
				<div className='tc-grid__left'>
					{/* Live Campaigns — P1.2: agent selects which offers to receive */}
					<section
						className='card card--spacious tc-card'
						aria-labelledby='tc-campaigns-title'
						style={{ maxHeight: "400px", overflowY: "scroll" }}>
						<div className='tc-card__head'>
							<div>
								<h2
									id='tc-campaigns-title'
									style={{
										margin: 0,
								...
								<p
									...
									Select the offers you want calls from. You only ring for
									campaigns marked live.
								</p>
							</div>
							<span
								className={`badge ${campaignsReady ? "badge-success" : "badge-warning"}`}
									...
									return (
										<div
											...
											style={{
												...
												padding: "10px 12px",
												borderRadius: 10,
												border: `1px solid ${live ? "rgba(34,197,94,0.28)" : "var(--line)"}`,
												background: live
													? "rgba(34,197,94,0.07)"
													: "rgba(255,255,255,0.02)",
											}}>
											...
											<button
												...
												disabled={busy}
												onClick={() => toggleLiveCampaign(c.id, !live)}
												aria-pressed={live}
												style={{ fontSize: 11, whiteSpace: "nowrap" }}>
												{busy ? "…" : live ? "Live ✓" : "Go live"}
											</button>
										</div>
									);
								})}
					...
					<section
						className='card card--spacious tc-card'
						aria-labelledby='tc-device-title'>
						<div className='tc-card__head'>
							<div>
								<h2
									id='tc-device-title'
								...
								<p
									className='text-muted'
									style={{ margin: "4px 0 0", fontSize: 12, lineHeight: 1.5 }}>
									Verify headset on this browser. We gate{" "}
									<span
										className='text-mono-sm'
										style={{ fontSize: 11, color: "var(--ink)" }}>
										Go Online
									</span>{" "}
									until mic & speaker are tested. Stored per device as{" "}
									<span className='text-mono-sm' style={{ fontSize: 10 }}>
										cc-device-ready
									</span>
									.
								</p>
							...
							<span
								className={`badge ${deviceReady ? "badge-success" : "badge-warning"}`}
								style={{ flexShrink: 0, fontSize: 10 }}>
						...
						<DeviceTest
							compact={false}
							agentId={agentId}
							onReadyChange={handleDeviceReady}
						/>
						...
						{!deviceReady && (
							<p
								...
								Tip: Click <strong>Start mic check</strong> and allow
								permission, speak for a few seconds, then{" "}
								<strong>Play test sound</strong>. Both must show ✓ TESTED.
							</p>
						)}
					</section>
					...
					{agentId && agentInfo && (
						<section
							...
							aria-labelledby='tc-snapshot-title'>
							<div className='tc-card__head'>
								<h2
									id='tc-snapshot-title'
									style={{
										margin: 0,
							...
							<div className='tc-snapshot-grid'>
								...
								<div className='tc-snapshot-field'>
									...
									<span
										className='call-detail-value text-mono-sm'
								...
								<div className='tc-snapshot-field'>
									...
									<span
										className='call-detail-value text-mono-sm'
										style={{ fontSize: 12 }}>
										{agentInfo.forwarding_number ?? "Not set"}
									</span>
								</div>
								<div className='tc-snapshot-field'>
								...
								<div
									...
									<div
										...
										{agentInfo.states?.length ? (
											agentInfo.states.slice(0, 8).map((c: string) => (
												<span
													key={c}
											...
											<span
												...
												Any state (no restriction)
											</span>
										)}
										{(agentInfo.states?.length ?? 0) > 8 && (
											<span className='badge' style={{ fontSize: 10 }}>
												+{(agentInfo.states?.length ?? 0) - 8} more
											</span>
										)}
									</div>
								</div>
								<div
									...
									<span
										className='call-detail-value text-mono-sm'
										style={{ fontSize: 11, color: "var(--muted)" }}>
										{agentInfo.last_assigned_at
											? new Date(agentInfo.last_assigned_at).toLocaleString(
													"en-GB",
													{
														day: "2-digit",
														month: "short",
							...
							{!pstnForwardingOk && (
								<p
									...
									PSTN fallback is selected but no forwarding number is set —
									routing may fail. Contact admin.
								</p>
							)}
						</section>
					)}
				...
				<div className='tc-grid__right'>
					...
					{agentId && (
						<section
							id='tc-states'
							className='card card--spacious tc-card'
							aria-labelledby='tc-states-title'>
							<div className='tc-card__head'>
								<h2
									id='tc-states-title'
									style={{
								...
								{!editingStates ? (
									<button
										className='btn btn-sm btn-ghost'
										onClick={() => {
											setStatesDraft([...(agentInfo?.states ?? [])]);
							...
							{editingStates ? (
								<div
									...
									<p
										...
										Pick states you are licensed to handle. Leave empty for “any
										state”. Used for state-wise routing — only calls from these
										states will route to you.
									</p>
									<div
										style={{
											display: "flex",
									...
									<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
										...
										{statesDraft.length > 0 && (
											<button
												className='btn btn-ghost btn-sm'
												onClick={() => setStatesDraft([])}>
												Clear
											</button>
										)}
									</div>
									<p
										className='text-mono-sm'
										style={{ fontSize: 10, color: "var(--muted)", margin: 0 }}>
										{statesDraft.length} selected ·{" "}
										{statesDraft.length === 0
											? "Any state"
											: statesDraft.join(", ").slice(0, 60) +
												(statesDraft.join(", ").length > 60 ? "…" : "")}
									</p>
								</div>
							) : (
								<>
									...
									<p
										...
										State filter is live for routing. Ask admin to adjust
										campaign allowed states if you are not receiving calls.
									</p>
								</>
							)}
						</section>
					...
					{!agentId && (
						<section className='card card--spacious tc-card'>
							<h2 style={{ margin: 0, font: "600 14px var(--serif)" }}>
								No agent profile
							</h2>
							<p
								className='text-muted'
								style={{ fontSize: 12, marginTop: 8, lineHeight: 1.6 }}>
								Create your agent profile to configure states and verify
								devices. An admin must approve you before routing begins.
							</p>
						</section>
					)}
			{error && (
			...
			{/* Recent Calls: right column, under Licensed States */}
			<section
				className='card card--table tc-calls-card'
				aria-labelledby='tc-calls-title'
				...
				<div className='card-header' style={{ gap: 16, alignItems: "center" }}>
					<div style={{ flex: "1 1 160px", minWidth: 0 }}>
						...
						<p
							...
							Your last routed calls. Search by ID, hash, state or campaign.
						</p>
					</div>
					<div
						style={{
						...
						<span
							...
							{filteredCalls.length} call(s){callDebounced ? " · filtered" : ""}
							{filteredCalls.length > PAGE_SIZE
								? ` · page ${callPage}/${callTotalPages}`
								: ""}
						</span>
					</div>
				</div>
					...
					{filteredCalls.length === 0 ? (
						<div className='empty-state' style={{ padding: "36px 24px" }}>
							...
							<p
								...
								{calls.length === 0
									? "Once you go online, inbound calls routed to you will appear here. Test devices and go online to start."
									: "Try a different search or clear the filter to see all calls."}
							</p>
							{callDebounced && (
								<button
									className='btn btn-sm btn-ghost'
