...
export default function AdminCalendarPage() {
	...
	const fetchAll = useCallback(async () => {
		...
		const [sRes, bRes, aRes] = await Promise.all([
			fetch("/api/v1/onboarding/slots"),
			fetch("/api/v1/onboarding/bookings"),
			fetch("/api/v1/agents?limit=200"),
		]);
		...
		if (aRes.ok) {
			...
			for (const ag of rows as any[]) {
				map[ag.id] = {
					name:
						ag.user_name ?? ag.membership_id?.slice(0, 8) ?? ag.id.slice(0, 8),
					email: ag.user_email ?? "",
				};
			}
			setAgentMap(map);
	...
	async function handleCreate() {
		...
		if (!Number.isFinite(cap) || cap < 1 || cap > 100) {
			showToast("Capacity 1..100", "error");
			return;
		}
		...
		setCreating(true);
		try {
			const res = await fetch("/api/v1/onboarding/slots", {
				method: "POST",
	...
	async function toggleActive(s: Slot) {
		const res = await fetch(`/api/v1/onboarding/slots/${s.id}`, {
			method: "PATCH",
			headers: { "Content-Type": "application/json" },
	...
	async function removeSlot(id: string) {
		...
		const res = await fetch(`/api/v1/onboarding/slots/${id}`, {
			method: "DELETE",
		});
		const b = await res.json().catch(() => ({}));
		if (res.ok) {
			...
			fetchAll();
		} else showToast(b.message ?? "Failed", "error");
	}
	async function setBookingStatus(id: string, status: string) {
		const res = await fetch(`/api/v1/onboarding/bookings/${id}`, {
			method: "PATCH",
			headers: { "Content-Type": "application/json" },
	...
	const filteredSlots = useMemo(() => {
		...
		out.sort((a, b) => {
			...
			else if (slotSortBy === "booked_count")
				cmp = a.booked_count - b.booked_count;
			else if (slotSortBy === "is_active")
	...
	const slotTotalPages = Math.max(
		1,
		Math.ceil(filteredSlots.length / PAGE_SIZE),
	);
	...
	useEffect(() => {
		setSlotPage(1);
	}, [slotSearch, slotDateFilter, slotStatusFilter]);
	useEffect(() => {
		if (slotPage > slotTotalPages) setSlotPage(slotTotalPages);
	}, [slotTotalPages, slotPage]);

	// Bookings filtered
	const filteredBookings = useMemo(() => {
		let out = [...bookings];
		if (bookingDateFilter)
			out = out.filter((b) => b.date === bookingDateFilter);
		if (bookingStatusFilter)
			out = out.filter((b) => b.status === bookingStatusFilter);
		...
		out.sort((a, b) => {
			...
			} else if (bookingSortBy === "created_at")
				cmp =
	...
	}, [bookingTotalPages, bookingPage]);

	function handleSlotSort(f: string) {
		if (slotSortBy === f) setSlotOrder((o) => (o === "asc" ? "desc" : "asc"));
		else {
			setSlotSortBy(f);
			setSlotOrder("asc");
		}
	}
	function handleBookingSort(f: string) {
		if (bookingSortBy === f)
			setBookingOrder((o) => (o === "asc" ? "desc" : "asc"));
		else {
			setBookingSortBy(f);
			setBookingOrder("asc");
		}
	...
	const slotColumns: Column<Slot>[] = [
		{
			...
			header: "Date",
			sortable: true,
			render: (s) => <span className='text-mono-sm'>{s.date}</span>,
		},
		{
			key: "time",
			header: "Time",
		...
		},
		{
			...
			render: (s) => (
				<span
					className={`badge ${s.booked_count >= s.capacity ? "badge-danger" : "badge-info"}`}>
	...
	const bookingColumns: Column<Booking>[] = [
		{
			...
			header: "Date",
			sortable: true,
			render: (b) => <span className='text-mono-sm'>{b.date}</span>,
		},
		{
			key: "time",
			header: "Time",
		...
		},
		{
			...
			render: (b) => {
				...
				return (
					<span className='text-mono-sm' title={b.agent_id}>
		...
		},
		{
			...
			render: (b) => (
				<span
					className={`badge ${b.status === "confirmed" ? "badge-success" : b.status === "cancelled" ? "badge-danger" : "badge-warning"}`}>
					{b.status}
				</span>
			),
		},
			...
			<div className='dashboard-page-header'>
				<div>
					...
					<p
						...
						Configure available dates/times for post-signup onboarding calls.
			...
			<div className='card card--table' style={{ marginTop: "var(--space-5)" }}>
				<div
					...
					<div style={{ display: "flex", gap: 8, alignItems: "center" }}>
						...
						<select
							...
							onChange={(e) => setSlotStatusFilter(e.target.value as any)}
				...
				{slots.length === 0 ? (
					<div style={{ padding: "var(--space-8) 16px", textAlign: "center" }}>
						<p style={{ font: "500 15px var(--serif)", margin: "0 0 6px" }}>
							No weekly slots yet
						</p>
						<p
							...
							Create your first onboarding slot above (date + start/end +
							capacity). Agents see active slots in{" "}
							<strong>Book Call → Onboarding</strong> and get a confirmation +
							1h reminder. Empty state is correct — not a bug.
						</p>
						<p
							className='text-mono-sm'
							style={{ fontSize: 10, color: "var(--muted)" }}>
							Tip: Add 2–3 weekly slots with capacity 3–5 each; use “Active”
							toggle to hide without deleting.
						</p>
					</div>
