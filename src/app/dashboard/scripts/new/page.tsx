      ...
      <form onSubmit={handleSubmit} className="card card--form" style={{ maxWidth: 680 }}>
        ...
        <div className="form-group">
          ...
          <span className="form-hint">
            Click a chip to insert at cursor. Also supports {"{{agent_name}}"}, {"{{phone}}"}, {"{{npn}}"}, {"{{state}}"}, {"{{beneficiary}}"}. Live preview below shows filled values.
          </span>
        </div>
        {content.trim() && (
          <div className="card" style={{ background: "rgba(168,85,247,0.06)", border: "1px solid var(--line)", padding: "var(--space-4)" }}>
            <p style={{ font: "600 10px var(--mono)", letterSpacing: "0.12em", color: "var(--muted)", margin: "0 0 8px" }}>LIVE PREVIEW — filled with sample agent</p>
            <pre style={{ whiteSpace: "pre-wrap", font: "13px/1.6 var(--mono)", margin: 0, color: "var(--ink)" }}>{preview}</pre>
          </div>
        )}
